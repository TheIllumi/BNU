...
const TOPICS = [
  ...
  { id: "t01", name: "Blockchain — Definition & Properties", week: "Week 1", priority: "high",
    ...
    <div class="mistake">⚠️ Common Mistake: Saying blockchain "encrypts" data. Bitcoin does NOT encrypt transactions — all records are public.</div>` },
  ...
  { id: "t02", name: "Cryptographic Hashing (SHA-256)", week: "Week 1", priority: "high",
    ...
    <div class="mistake">⚠️ Hashing ≠ Encryption. Hashing is one-way; encryption is two-way (with a key).</div>` },
  ...
  { id: "t03", name: "Hashing vs Encryption", week: "Week 1", priority: "high",
    ...
    <div class="mistake">⚠️ This is Quiz 1 Q4 (2 marks) — know this cold.</div>` },
  ...
  { id: "t04", name: "Digital Signatures", week: "Week 1", priority: "high",
    ...
    <div class="mistake">⚠️ Never say you sign with the public key. Sign = private. Verify = public.</div>` },
  ...
  { id: "t06", name: "Proof of Work (PoW)", week: "Week 2", priority: "high",
    ...
    <div class="mistake">⚠️ Hash must be LESS THAN OR EQUAL TO the target, not equal to.</div>` },
...
Hash01  = SHA256(SHA256(Hash0+Hash1)) // parent</pre>
    ...
    <div class="mistake">⚠️ Common mistake: drawing root at the bottom. Leaves (Tx0..Tx3) at bottom; root at TOP, in the header.</div>` },
  ...
  { id: "t10", name: "Block Structure (80-byte header)", week: "Week 2", priority: "high",
    ...
    <div class="mistake">⚠️ Full transaction list is NOT in the 80-byte header. The Merkle ROOT is. (Quiz trap.)</div>` },
...
Leading zeros: 8 hex zeros = 32 bits</pre>
    ...
    <div class="mistake">⚠️ Use (exponent − 3), NOT exponent. Don't mix decimal and hex.</div>` },
...
P = 10^9 H/s: n ≈ 39.1 → n ≈ 39  (difference ≈ 10)</pre>
    ...
    <div class="mistake">⚠️ Don't forget the 600! And use log base 2, not log base 10 (divide by log10(2) ≈ 0.301 if needed).</div>` },
...
difficulty = (65535/378681) × 2^(8×(29−23)) = 0.173 × 2^48 ≈ 4.88×10^13</pre>
    ...
    <div class="mistake">⚠️ difficulty = reference/current, not current/reference!</div>` },
...
1 gwei = 10^9 wei</pre>
    <div class="mistake">⚠️ If transaction needs more than gas limit → it FAILS.</div>` },
  ...
  { id: "t24", name: "Contract Creation vs Invocation", week: "Week 11", priority: "high",
    ...
    <div class="mistake">⚠️ to:none = create; to:address = invoke. MSTORE opcode = 0x52; SSTORE = 0x55.</div>` },
...
function updateDashboard() {
  ...
  if (recommended.length === 0) {
    sn.innerHTML = '<div style="color:var(--success);font-size:.85rem">🎉 All topics completed! Great work.</div>';
  } else {
  ...
  const html = list.map(t => {
    ...
    <div class="topic-item ${ts.status}" onclick="openTopic('${t.id}')">
      ...
      <div class="topic-badges">
        ${ts.revision ? '<span class="badge badge-revision">⭐ Rev</span>' : ""}
...
function openTopic(id) {
  ...
  $("modalRevisionToggle").textContent = ts.revision ? "⭐ Remove Revision" : "⭐ Add to Revision";
...
function renderMCQCard() {
  ...
  if (hist) {
    ...
    exp.innerHTML = `<strong>${hist.correct ? "✅ Correct!" : "❌ Incorrect."}</strong> ${mcq.exp}`;
  } else {
    ...
    <div class="q-item">
      <div class="q-header" onclick="toggleAnswer('${q.id}')">
        ...
        <div class="q-actions">
          ...
          <button class="q-toggle-btn ${qs.revision ? 'revision-active' : ''}" onclick="toggleQState(event,'${q.id}','revision')">⭐</button>
        </div>
      </div>
  ...
  const html = DIAGRAMS.map(d => {
    ...
    <div class="diagram-card">
      <div class="diagram-header" onclick="toggleDiagram('${d.id}')">
        <div class="diagram-title">${ds.status === "memorized" ? "✅" : ds.status === "needs-practice" ? "⚠️" : "☐"} ${d.name}</div>
      ...
      <div class="diagram-body" id="db-${d.id}">
        ...
        <div class="diagram-actions">
          ...
          <button class="d-btn ${ds.status === 'needs-practice' ? 'needs-practice' : ''}" onclick="setDiagramStatus('${d.id}','needs-practice')">⚠️ Needs Practice</button>
...
function setDiagramStatus(id, status) {
  ...
  renderDiagrams();
  updateDashboard();
  showToast(status === "memorized" ? "✅ Marked as memorized!" : status === "needs-practice" ? "⚠️ Flagged for practice" : "↺ Reset");
}
  ...
  const html = QUESTIONS.calc.map((c, i) => {
    ...
    <div class="calc-item">
      <div class="calc-header" onclick="toggleCalc('${c.id}')">
        ...
        <span class="calc-status-badge">${cs.done ? "✅" : "☐"}</span>
      </div>
...
function renderRevision() {
  ...
  $("revDiagrams").innerHTML = REVISION.diagrams.map(d => {
    return `<div class="rev-diagram-item">📐 ${d}</div>`;
  }).join("");
